...
export default {
  ...
  mounted() {
    ...
    let drawimgarr = [
      {
        name: "bg",
        url: "https://img.xuanyiai.com/mini_fortune_canvas_bg1.jpg"
      },
  ...
  methods: {
    ...
    init() {
      ...
      this.$HTTP
        .fortune_report({
          report_id: this.query
        })
        .then(res => {
          let result = res.data;
          console.log(result);
          that.birthCode = result.life_number;
          that.price = result.report_price;
          that.fortuneDetail = result;
          that.buyStatus = result.is_locked ? 0 : 1;
    ...
    createCanvas() {
      ...
      ctx.fillText(
        code.i,
        74 + 214 / 5 - Math.ceil(ctx.measureText("7").width / 2),
        758
      );
      ctx.fillText(
        code.j,
        74 + (214 / 5) * 2 - Math.ceil(ctx.measureText("5").width / 2),
        758
      );
      ctx.fillText(
        code.k,
        74 + (214 / 5) * 3 - Math.ceil(ctx.measureText("1").width / 2),
        758
      );
      ctx.fillText(
        code.l,
        74 + (214 / 5) * 4 - Math.ceil(ctx.measureText("1").width / 2),
        758
      );
      // 三角形-第二层
